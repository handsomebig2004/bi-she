...
class ConvBranch1D(nn.Module):
    def __init__(
        ...
        blocks = []
        for in_ch, out_ch, kernel in zip(channels[:-1], channels[1:], kernels):
            blocks.extend(
                [
...
def train_one_fold(
    ...
    model = LateFusionCNN1D(n_classes=n_classes, dropout=dropout).to(device)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Late-fusion EDA+BVP CNN LOSO on UNIVERSE.")
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path(r".\data\UNIVERSE\windows_eda_bvp_latefusion_60s30s"),
    )
...
def main():
    ...
    print(f"\n=== Late Fusion {task_name} LOSO Summary (KEPT folds only) ===")
