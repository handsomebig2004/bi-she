...
def resample_1d_linear(x: np.ndarray, target_len: int) -> np.ndarray:
    """Linearly resample 1D array to target_len."""
...
def make_pair_key(df: pd.DataFrame) -> pd.Series:
    """Robust pairing key using subject/session/task/start/end."""
...
class EarlyFusionCNN1D(nn.Module):
    """
    Early fusion CNN for 2-channel input:
      channel 0 = EDA (resampled)
      channel 1 = BVP
    """
...
def fit_standardizer(X: np.ndarray):
    """
    X: [N, T]
    return mean, std as scalars computed on training fold only
    """
...
def train_one_fold(
    ...
    # fold-wise standardization, separately for each modality
    ...
    train_loader = DataLoader(
        EarlyFusionDataset(X_eda_tr, X_bvp_tr, y_tr),
        batch_size=batch_size, shuffle=True, num_workers=0
    )
    val_loader = DataLoader(
        EarlyFusionDataset(X_eda_va, X_bvp_va, y_va),
        batch_size=256, shuffle=False, num_workers=0
    )
    test_loader = DataLoader(
        EarlyFusionDataset(X_eda_test, X_bvp_test, y_test),
        batch_size=256, shuffle=False, num_workers=0
    )
...
def load_and_pair_data(eda_dir: Path, bvp_dir: Path):
    ...
    if len(common_keys) == 0:
        raise RuntimeError("EDA 和 BVP 没有配对上的窗口，请检查 meta.csv 的 key 字段。")
    ...
    if not np.array_equal(y1, y2):
        ...
        raise RuntimeError(f"配对后发现标签不一致，mismatch count = {len(mismatch)}")
...
def main():
    ...
    out_dir = Path(r".\data\UNIVERSE\windows_eda_bvp_earlyfusion_60s30s")
