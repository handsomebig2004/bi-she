...
class ResNetBranch1D(nn.Module):
    def __init__(
        ...
        if pool == "attn":
            self.pool = AttentionPool1D(in_channels)
        elif pool == "avg":
...
class LateFusionResNet1D(nn.Module):
    def __init__(self, n_classes: int = 3, dropout: float = 0.25, pool: str = "avg"):
        super().__init__()
...
def train_one_fold(
    ...
    model = LateFusionResNet1D(n_classes=n_classes, dropout=dropout, pool=pool).to(device)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Optimized binary late-fusion EDA+BVP ResNet1D LOSO on UNIVERSE.")
    ...
    parser.add_argument("--pool", choices=("avg", "attn"), default="attn")
...
def main():
    ...
    X_eda, X_bvp, y, meta = load_and_pair_data(args.eda_dir, args.bvp_dir, binary=True)
    groups = meta["subject"].astype(str).values
    subjects = np.unique(groups)

    print("Optimized Binary paired windows:", len(y))
    ...
    print(f"\n=== Optimized Late Fusion ResNet Binary LOSO Summary ({args.config_name}) ===")
