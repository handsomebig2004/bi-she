...
def compute_minmax_from_baseline(
    session_root: Path,
    ...
    """
    Use baseline EDA to compute robust min/max for subject/session normalization.
    ...
    lo = float(np.quantile(xb, q_low))
    hi = float(np.quantile(xb, q_high))
...
def parse_args():
    parser = argparse.ArgumentParser(description="Build UNIVERSE EDA windows.")
...
def main():
    ...
    if out_dir is None:
        out_dir = universe_root / f"windows_eda_{args.win_s}s{args.hop_s}s"
    ...
    mm_cache: Dict[Tuple[str, str], Tuple[float, float] | None] = {}
    ...
    for session in sessions:
        ...
        for p in glob.glob(pattern):
            ...
            # baseline min-max (compute once per subject/session)
            if key not in mm_cache:
                mm_cache[key] = compute_minmax_from_baseline(
            ...
            mm = mm_cache[key]
            if mm is None:
                print(f"{subject_id}/{session_name} 找不到可用 EDA baseline(relaxation_video)，跳过该 session。")
                continue

            lo, hi = mm

            x = load_eda_series(pkl_path)
            x = user_minmax_normalize(x, lo, hi, clip=True)

            windows, starts = sliding_windows_1d(x, win=win, hop=hop)

            if len(windows) == 0:
                print(
                    f"{subject_id}/{session_name}/{task_name} 的 EDA 数据长度 {len(x)} "
    ...
    if not X_list:
        raise RuntimeError("没有找到任何有效的 EDA 窗口，请检查路径、标签和 baseline 配置。")
