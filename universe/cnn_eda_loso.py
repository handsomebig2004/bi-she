...
def evaluate(model, loader, device, n_classes=3):
    ...
    # Make metrics stable even when a class is missing
...
def train_one_fold(X, y, groups, test_subject, seed=0, epochs=30, lr=1e-3, batch_size=64):
...
def main():
    # -------- config --------
    data_dir = Path(r".\data\UNIVERSE\windows_eda_60s30s")  # 改成你的实际目录
    min_test_windows = 50
    ...
    for i, sub in enumerate(subjects, 1):
        # quick pre-check before training this fold:
        ...
        if n_test < min_test_windows:
            skipped.append({"subject": sub, "reason": f"n_test<{min_test_windows}", "n_test": int(n_test), "classes": uniq.tolist()})
            print(f"[{i:02d}/{len(subjects)}] subject={sub} SKIP (n_test={n_test} < {min_test_windows})")
            continue

        if len(uniq) < min_unique_classes:
            skipped.append({"subject": sub, "reason": f"unique_classes<{min_unique_classes}", "n_test": int(n_test), "classes": uniq.tolist()})
            print(f"[{i:02d}/{len(subjects)}] subject={sub} SKIP (classes={uniq.tolist()})")
            continue

        best_val_f1, test_f1, test_ba, cm, n_test2, uniq2 = train_one_fold(X, y, groups, test_subject=sub, seed=100+i)
        kept.append({
            "subject": sub,
            "n_test": int(n_test2),
        ...
        cm_sum += cm
        print(f"[{i:02d}/{len(subjects)}] subject={sub} "
